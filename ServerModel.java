...
public final class ServerModel {
    ...
    private TreeMap<Integer, String> registeredUsers;
    private TreeMap<String, Channel> channels;

    public ServerModel() {
        registeredUsers = new TreeMap<Integer, String>();
    ...
    public int getUserId(String nickname) {
        if (registeredUsers.containsValue(nickname)) {
            Set<Map.Entry<Integer, String>> entries = registeredUsers.entrySet();
    ...
    public Collection<String> getRegisteredUsers() {
        ...
        Set<Map.Entry<Integer, String>> u = registeredUsers.entrySet();
        for (Map.Entry<Integer, String> entry : u) {
    ...
    public Collection<String> getUsersInChannel(String channelName) {
        ...
        Collection<String> usersInChannel = channels.get(channelName).getUsers();
        Collection<String> users = new TreeSet<>(usersInChannel);
        return users;
    }
    ...
    public Broadcast deregisterUser(int userId) {
        ...
        TreeSet<String> removedChannels = new TreeSet<String>();
        String u = registeredUsers.get(userId);
        if (registeredUsers.containsKey(userId)) {
            Set<Map.Entry<String, Channel>> chan = channels.entrySet();
            for (Map.Entry<String, Channel> entry : chan) {
                if (entry.getValue().containsUser(u)) {
                    recipients.addAll(entry.getValue().getUsers());
                    if (entry.getValue().getOwner().equals(u)) {
                        // channels.remove(entry.getKey());
                        removedChannels.add(entry.getKey());
                    } else {
                        entry.getValue().removeUser(u);
                    }
                }
            }
            for (String c : removedChannels) {
                channels.remove(c);
            }
    ...
    public Broadcast changeNickname(NicknameCommand nickCommand) {
        if (registeredUsers.values().contains(nickCommand.getNewNickname())) {
        ...
        TreeSet<String> recipients = new TreeSet<String>();
        String u = registeredUsers.get(nickCommand.getSenderId());
        ...
        for (Map.Entry<String, Channel> entry : chan) {
            if (entry.getValue().containsUser(u)) {
                ...
                entry.getValue().addUser(nickCommand.getNewNickname());
            }
            if (entry.getValue().getOwner().equals(nickCommand.getSender())) {
                entry.getValue().setOwner(nickCommand.getNewNickname());
            }
        }

        registeredUsers.replace(nickCommand.getSenderId(), nickCommand.getNewNickname());
    ...
    public Broadcast createChannel(CreateCommand createCommand) {
        if (channels.keySet().contains(createCommand.getChannel())) {
        ...
        Collection<String> recipients = new LinkedList<String>();
        String channelName = createCommand.getChannel();
        String userName = createCommand.getSender();
        boolean priv = createCommand.isInviteOnly();

        Channel c = new Channel(userName, priv, channelName);
    ...
    public Broadcast joinChannel(JoinCommand joinCommand) {
        if (!(channels.keySet().contains(joinCommand.getChannel()))) {
            return Broadcast.error(joinCommand, ServerResponse.NO_SUCH_CHANNEL);
        }
        Channel c = channels.get(joinCommand.getChannel());
        if (c.getPrivacy()) {
    ...
    public Broadcast sendMessage(MessageCommand messageCommand) {
        ...
        Channel c = channels.get(messageCommand.getChannel());
        if (!(c.getUsers().contains(messageCommand.getSender()))) {
    ...
    public Broadcast leaveChannel(LeaveCommand leaveCommand) {
        ...
        Channel c = channels.get(leaveCommand.getChannel());
        if (!(c.getUsers().contains(leaveCommand.getSender()))) {
            return Broadcast.error(leaveCommand, ServerResponse.USER_NOT_IN_CHANNEL);
        }
        TreeSet<String> recipients = new TreeSet<String>();
        recipients.addAll(c.getUsers());
        if (c.getOwner().equals(leaveCommand.getSender())) {
            channels.remove(leaveCommand.getChannel());
        } else {
            c.removeUser(leaveCommand.getSender());
        }
    ...
    public Broadcast inviteUser(InviteCommand inviteCommand) {
        if (!registeredUsers.values().contains(inviteCommand.getUserToInvite())) {
            return Broadcast.error(inviteCommand, ServerResponse.NO_SUCH_USER);
        }
        if (!(channels.keySet().contains(inviteCommand.getChannel()))) {
            return Broadcast.error(inviteCommand, ServerResponse.NO_SUCH_CHANNEL);
        }
        Channel c = channels.get(inviteCommand.getChannel());
        if (!(c.getOwner().equals(inviteCommand.getSender()))) {
        ...
        c.addUser(inviteCommand.getUserToInvite());
        TreeSet<String> recipients = c.getUsers();

        return Broadcast.names(inviteCommand, recipients, c.getOwner());
    }
